...
namespace CibraryEngine
{
	...
	struct Sphere
	{
		public:
			...
			static Sphere Expand(const Sphere& a, const Sphere& b);
			/** Returns the minimum sphere able to contain both the sphere and point given */
			static Sphere Expand(const Sphere& a, const Vec3& b);

			/** Determines whether two spheres intersect */
			static bool IntersectTest(const Sphere& a, const Sphere& b);
			/** Determines whether one sphere occludes another, given the position of the viewer... this may not be very useful. */
			static bool OcclusionTest(const Sphere& occluding, const Sphere& hidden, const Vec3& viewer);
	};
}
